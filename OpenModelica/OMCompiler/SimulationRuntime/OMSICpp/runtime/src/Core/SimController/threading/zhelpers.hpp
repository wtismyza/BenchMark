...
static std::string
s_recv (zmq::socket_t & socket,bool wait =true) {
...
static bool
s_send (zmq::socket_t & socket, const std::string & string, bool wait = true) {
    ...
    {
        rc = socket.send(message, ZMQ_DONTWAIT);
    }
    return (rc);
}
...
static bool
s_sendmore (zmq::socket_t & socket, const std::string & string, bool wait = true) {
    ...
    {
        rc = socket.send(message, ZMQ_SNDMORE| ZMQ_DONTWAIT);
    }
    return (rc);
}
